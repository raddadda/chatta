...
db.Board.belongsTo(db.User,{foreignKey:'poster_id' , targetKey:'user_id' , onDelete:'CASCADE'})
// 유저 - 게시판 의   1:다 관계  (작성한 게시판)


db.User.hasMany(db.Board_Bookmark,{foreignKey:'user_id'});
db.Board.hasMany(db.Board_Bookmark,{foreignKey:'id'})
db.Board_Bookmark.belongsTo(db.User,{foreignKey:'user_id',onDelete:'CASCADE'});
db.Board_Bookmark.belongsTo(db.Board,{foreignKey:'board_id',targetKey:'id',onDelete:'CASCADE'})
...
db.Chat_Room.hasMany(db.Chat_Message,{foreignKey:'id'})
db.Chat_Message.belongsTo(db.User,{foreignKey:'user_id'});
db.Chat_Message.belongsTo(db.Chat_Room,{foreignKey:'room_id',targetKey:'id',onDelete:'CASCADE'})
